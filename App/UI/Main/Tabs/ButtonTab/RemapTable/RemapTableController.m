...
- (void)viewDidLoad {
    ...
    if (@available(macOS 26.0, *)) {
        ...
        if (runningPreRelease()) dispatch_async(dispatch_get_main_queue(), ^{ if (![[MainAppState.shared.window.contentView valueForKey:@"prefersCompactControlSizeMetrics"] boolValue]) assert(false); });
    }
    else
